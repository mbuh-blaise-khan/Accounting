...
Covers:
  ...
  3. OHADA income statement (Compte de resultat) ordinary result matches
...
def test_ohada_income_statement_separates_class8_hao(client):
...
def test_reversed_pair_nets_to_zero_and_drafts_excluded(client):
    ...
    assert "Purchases of goods - local" not in is_names  # reversed -> zero
    ...
    assert float(fb["assets"]) == float(fb["liabilities"] + fb["equity"])
