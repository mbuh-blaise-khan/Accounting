...
## Post-S9 round 5 — Business Profile Session 2: organization purpose, business activity, accounting basis, company description
- [x] `organizations` gained ALL-OPTIONAL `org_purpose` (for_profit / non_profit / ngo_association / government), `business_activity` (category code or free text, max 120), `accounting_basis` (accrual/cash — NOT NULL with server default 'accrual' because a basis always exists implicitly) and `company_description` (text, max 1000) via migration `0013`. Pre-existing orgs remain valid; `accounting_basis` backfills to 'accrual'.
- [x] ALL FOUR are purely informational: ZERO effect on posting, ledger, trial-balance or statement computation (the engine is accrual-based and never reads these fields — documented in model/schema/service).
- [x] Backend: `OrganizationUpdate` accepts the four fields (blank strings clear text fields; bare "OTHER" activity sentinel rejected 422 — the free-text description replaces it); Session-2 tests in `test_business_profile.py` (set → read back → clear text fields, enum persistence, OTHER sentinel rejection, defaults).
- [x] Known backend limitation (documented, deliberate): `org_purpose` is enum-backed and the PATCH route uses `exclude_unset` + "None = no change", so a chosen purpose can be CHANGED but not UNSET via the API; text fields DO clear via empty string. Future enhancement if ever needed.
- [x] Frontend `BusinessProfilePage` (rebuilt as ONE complete file after a prior session's incremental-edit corruption truncated it): purpose radio group (2-col on wider screens, stacked on phone), sector `SearchSelect` reusing the shared component (12 sectors; "Other" reveals a free-text field, max 120), accrual/cash radios with one-line explanations + a visible amber "informational only — does not change any accounting calculation" note, and a company-description textarea (max 1000, live char counter). All four NEVER block saving in either mandatory or settings mode; all existing Part-2 behaviour (identity radios, framework-aware country/legal-form SearchSelects, RCCM/tax gating, fiscal-year dropdown, address, validation) preserved.
- [x] `business_activity` storage contract: fixed sectors stored as the CATEGORY CODE; "Other" stores the free-text description. `ReportHeader` translates known codes back to localized labels (free text displays as-is), so reports read naturally in the active language.
- [x] i18n: consolidated the duplicated/orphan `bp.*` keys left by two prior attempts into ONE canonical set (78 `bp.*` keys, EN/FR parity verified with a node script; new key `bp.clearSelection`). No hardcoded strings in the page.
- [x] Verification observed (output captured to files — the sandbox terminal output could not be observed directly this session): `npm run build` ✓ 57 modules transformed, built in 7.25s, RC=0; backend full suite `pytest app/tests -q` **104 passed**, 10 pre-existing deprecation warnings only, RC=0 in 39.74s.
...
### Addendum — Session 13: Learn/Practice choice screen fixes (persistent hub, navigation, register-no-auto-login)
- [x] REGISTER-NO-AUTO-LOGIN (decision documented): `POST /auth/register` no longer sets the session cookie — registration creates the account ONLY; the user must actively log in. Chosen at the BACKEND over frontend-logout-after-register because the endpoint's contract should be "create an account", not "establish a session" (an httpOnly cookie can only be cleared server-side, so the workaround would leave a misleading contract and a brief real session on every signup). Frontend: Register → redirect to Login with a green "Account created — please log in" notice (EN+FR). Tests: `test_register_success_creates_account_without_session` (no cookie, `GET /me` → 401) + `test_login_after_register_establishes_session`; all test `_register` helpers explicitly log in after registering. Suite: **128 passed, RC=0**.
- [x] PERSISTENT HUB: EVERY user (new or existing) lands on the Learn/Practice choice screen immediately after logging in, EVERY time — no longer gated on whether they have workspaces. The "Both" option was REMOVED entirely (Learn and Practice only, plus "Show me around"); `choice.both*` i18n keys deleted in EN+FR.
- [x] PERSISTENT HUB ACCESS: "← Back to start" / logo-click in the authed header, in the Learn flow, and in the WorkSpace nav — reachable from anywhere in Learn or Practice. Both flows stay mounted-but-hidden when returning to the hub, so progress and data are never reset (resumes exactly where the user left off; Learn progress is additionally server-side).
- [x] LEARN NAV FIXED (root cause: `learnLessonId` was stored but no render branch ever consumed it): lesson cards on the standalone Learn page now actually open `LessonDetailPage`; the back arrow to the hub no longer triggers a refetch that previously hung on a permanent spinner.
- [x] PRACTICE NAV FIXED (root cause: DashboardPage's onboarding effect immediately cleared the choice, so the hub's gate bounced the user straight back): entering Practice now goes through the CORRECT new-vs-returning flow — zero workspaces → create-workspace → MANDATORY Business Profile step → practice space; one or more workspaces → "Your Workspaces" list (framework badges) + "+ Create a new business"; a workspace with an already-completed profile opens straight into its practice space (the profile form is re-demanded only when the server-side `profile_completed` flag is genuinely false).
- [x] AI HELP-BOT: deliberately DEFERRED to a possible future session (explicitly out of scope here; no AI-based logic built, per project rules). "Show me around" instead opens a lightweight 3-step WelcomeTour modal/stepper (welcome → Learn mode → Practice mode, EN+FR) whose final CTA creates the sample demo business and enters its practice space (the mandatory Business Profile step applies — server-side enforced, not bypassed).
- [x] Full verification: backend **128 passed** RC=0 (30.53s), frontend build **70 modules** RC=0 (26.99s).
